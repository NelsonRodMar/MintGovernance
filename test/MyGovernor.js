...
describe("MyGovernor", function () {
    ...
    describe("after proposing", () => {
        ...
        describe("after voting", () => {
            async function afterVotingFixture() {
                ...
                // wait for the 30 block voting period
    ...
    describe("after proposing new governor", () => {
        async function afterProposingNewGovernorFixture() {
            const deployValues = await deployFixture();
            const {governor, token, owner} = deployValues;
            ...
            const tx = await governor.propose(
                [token.address],
                [0],
                [token.interface.encodeFunctionData("changeGovernor", [owner.address])],
                "Change the governor!"
            );
            const receipt = await tx.wait();
            const event = receipt.events.find(x => x.event === 'ProposalCreated');
        ...
        it("should set the initial state of the proposal", async () => {
            const {governor, proposalId} = await loadFixture(afterProposingNewGovernorFixture);

            const state = await governor.state(proposalId);
            assert.equal(state, 0);
        });

        describe("after voting new governor", () => {
            async function afterVotingNewGovernorFixture() {
                const proposingValues = await afterProposingNewGovernorFixture();
                const {governor, proposalId} = proposingValues;
            ...
            it("should have set the vote", async () => {
                const {voteCastEvent, owner} = await loadFixture(afterVotingNewGovernorFixture);
            ...
            it("should allow executing the proposal", async () => {
                const {governor, token, owner} = await loadFixture(afterVotingNewGovernorFixture);

                await governor.execute(
                    [token.address],
                    [0],
                    [token.interface.encodeFunctionData("changeGovernor", [owner.address])],
                    keccak256(toUtf8Bytes("Change the governor!"))
                );

                const newGovernor = await token.governor();
                assert.equal(newGovernor.toString(), owner.address);
            });
        });
    });
});
